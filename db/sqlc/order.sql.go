...
const existOrderByIdentifier = `-- name: ExistOrderByIdentifier :one
SELECT EXISTS(
SELECT 
...
func (q *Queries) ExistOrderByIdentifier(ctx context.Context, identifier int32) (bool, error) {
	row := q.db.QueryRowContext(ctx, existOrderByIdentifier, identifier)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
...
const updateOrderIdentifier = `-- name: UpdateOrderIdentifier :one
UPDATE
    orders
SET
    identifier = $2
...
type UpdateOrderIdentifierParams struct {
	ID         uuid.UUID `json:"id"`
	Identifier int32     `json:"identifier"`
}

func (q *Queries) UpdateOrderIdentifier(ctx context.Context, arg UpdateOrderIdentifierParams) (Order, error) {
	row := q.db.QueryRowContext(ctx, updateOrderIdentifier, arg.ID, arg.Identifier)
	var i Order
	err := row.Scan(
		&i.ID,
		&i.Identifier,
